...
/**
 * This is the model class for table "tblemp_medical_certificate".
 *
 ...
 */
class EmployeeMedicalCertificate extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return 'tblemp_medical_certificate';
    }
    ...
    {
        return $this->hasOne(EmployeeMedicalCertificateId::className(), ['emp_id' => 'emp_id', 'from_date' => 'from_date']);
    }

    public function getFilePath()
    {
        // Construct the file path
        $filePath = Yii::getAlias('@frontend/web/upload_files/medical_cert/'.$this->filename.$this->filetempname);
        $path = '';
        // Check if the file exists
        $path .= file_exists($filePath) && !is_null($this->filename) ? Html::a($this->filename, Yii::getAlias('@web').'/upload_files/medical_cert/'.$this->filename.$this->filetempname, ['download' => true, 'data-pjax' => 0]).'<br>' : '';
